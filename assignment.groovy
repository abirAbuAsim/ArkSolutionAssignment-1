...
def formatedMargins = margins.inject([:]){result,margin->
...
def findCategory = {productPrice->((String)categoryMap.floorEntry(productPrice as BigDecimal)).split('=')[1]}//Find Product Category By Their Price
def calculateProductPriceWithMargin = {productPrice-> productPrice.multiply((1.plus(formatedMargins[findCategory(productPrice)])))} //Calculate Product Price With Margin Value
//Find Porduct Price With Margin By Group
def groupProductPriceWithMargin = products.inject(['G1':[],'G2':[],'G3':[]]){result,product->
 result[product[1]].push(calculateProductPriceWithMargin(product[2]))
 result
}
//Find Average Product Price By Group
def avgGroupProductPrice = groupProductPriceWithMargin.inject([:]){result,price->
